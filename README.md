...
因为传入框架的select方法的参数都会被变为字符串类型。也就是说，如果采用这样的方式：`sql="LIMIT"+currentPage+","+pageSize` 这种方式会造成传入的`currentPage`为非int类型，而是字符串类型。
...
想到我实习的技术栈和我现在的不一样，而且毕业了可能不会用到这些技术 。我觉得还是不要被这个所干扰，大概了解一下.Net，实习的时候好好学就行。记住自己的核心竞争力：认真学习设计模式，认真学习算法题，认真学习框架的原理。

原理很重要。我现在还不知道该如何入手所谓的”原理“。
...
目前我是这么打算的，别在低落的情绪中浪费时间，我的时间还是比较充裕的，因为研究生有三年，至少目前可以学习自己感兴趣的技术，即使很功利，即使不懂原理。

高兴起来！

真的很难高兴起来。我觉得这个非全日制给我的心里压力太大了。

1. 记录一个小的知识点：MetaData，元数据。在jdbc中常见。

   > NameNode实际上就是**管理文件的各种信息**（这种信息专业点我们叫做**MetaData**「元数据」），其中包括：文件路径名，每个Block的ID和存放的位置等等。
   
2. 学习了一下中间件的持久化原理，发现其实思想都是一样的。我觉得最初设计者的考量是这样的：设计中间件的目的是提高资源的相应速度，但是有了速度不可避免地会牺牲稳定性（可靠性）。于是为了两者兼顾，将**速度**和**可靠性**分开解决，两者独立工作：将数据存储在内存中，提高速度，然后将数据顺序写入磁盘，完成持久化的工作，具体来说是刷进各种名称不同的”log“里：

   > 先写buffer，然后**顺序IO**写Log。防止buffer的数据还没刷到磁盘，宕机导致数据丢失。
   >
   > ----来自[三歪吐血总结了各个中间件是如何实现持久化的](https://mp.weixin.qq.com/s?__biz=MzI4Njg5MDA5NA==&mid=2247490818&idx=1&sn=ad64b3d80b3fd5060860588802a13fdb&chksm=ebd75c03dca0d515d67a04198a34e2a1a4857b5e319983ee62027f31997fee6308f227ca4562&token=67449918&lang=zh_CN#rd)

3. [「只需7分钟！我将抢走你的赞」大数据入门](https://mp.weixin.qq.com/s?__biz=MzI4Njg5MDA5NA==&mid=2247486604&idx=1&sn=fba7a7f34c8faf182b88b137ff85b3ed&chksm=ebd74d8ddca0c49b87fc50027d94b272b7df7ac11e966a6a85b7c1f9bafec846f25a3c6461ef&token=2140209384&lang=zh_CN#rd) 这篇对大数据的”科普“写得比较好了，让我觉得这个方向还是很有意思的。总结一下关键就只有两点：存储和计算：**分布式存储**（单机难以存储的巨量数据），**移动计算到存储**和移动存储到计算，
...
上面这两个技术原理是目前遇到的两座大山，攻克困难。

##### 2020/12/26 Saturday

学习spring IOC的原理太难受了，我现在要找一个简单的只是进行学习，不然真的遭不住

##### 2020/12/27 Sunday

其实可以试着每天晚上复习leecode的题目，注意是复习，复习以前写过的题。

再规划一下这几天的技术学习路线：学学redis吧我服了，真TM福气而且服气，到处都是这个玩意儿，我怕了行吗，TM把你安排了行吗；在学学docker吧，也是个热门技术。 

至于Spring IOC的源码，明天或者后天再啃一啃，[链接在此](https://zhuanlan.zhihu.com/p/29344811)。这是块硬骨头，慢慢啃。

:star:发现一个新的方向：**数据挖掘**，可以试试。

##### 2020/12/28 Monday

复习了一下reactor模式，资料文件是在netty.md文件中，这是因为我在学习redis的时候发现这种模式在被广泛使用。总结起来它的优势在于：**将耗时操作（比如读取数据）和业务进程（handler）相分离，这样handler内部就不会有等待了。**

复习了一下回溯法设计实现全排列和动态规划实现零钱兑换问题。

##### 2021/1/10 Sunday

这不是一个正确的选择！

我开始怀疑我的选择了，我不要来这个石油单位实习的，我甚至不应该选择非全的！我他妈太菜了，我有点坚持不住了！我以后该怎么办，我好害怕。

what is past is prologue，过去，是美好未来的序章，还是艰难未来的序章？

rage, rage against the dying of the light. 我难道真的是在怒斥、反抗注定的悲剧吗？

我想看到希望，这样的黑暗难以忍受。

##### 2021/1/11 Monday

到钻采院实习的第一天，居然忘带药了，有点怕坚持不过后半场:joy:

分配的任务还是那么的无聊，我把这次实习当作兼职吧，重点还是得放在学习自己的知识上面。

我期待好消息，期待希望。

我感觉我还是没有汲取考研复试的教训，连找个实习都要找个关系，我是不是脑子有病。

##### 2021/1/13 Wednesday

Do not go gentle into that good night

我已经有点后悔选择这次实习，我宁愿拿这段时间来学车。但是呀，有三个字我至今没有好好理解：向前看。我没有那么多时间自怨自艾。
