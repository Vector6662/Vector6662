...
:star: :star: :star:结合下面的内容，发现异步（单线程下的）和多线程都是实现高并发的解决方案，而并发是一个**现象**：短时间内遇到大量操作请求。

##### 进程-->线程-->并发（这个地方问题十分严重，概念需要清晰）

目的都是为了实现并行或者并发（这里还没明确下来，再查查资料），但是异步最为轻量

> 并行，指同一时刻内多任务**同时**进行。边煮饭，边烧水，可以同时进行
>
> 并发，指在同一时间段内，多任务同时进行着，**但是某一时刻，只有某一任务执行**。边吃饭边喝水，同一时间点只能喝水和吃饭。

> :star: :star: :star:回调函数和事件处理程序本质上并无区别，只是在不同情况下，不同的叫法。



#### :heavy_exclamation_mark: :heavy_exclamation_mark: Promise机制
...
我大概明白了async/await为什么说是最高境界，实现了同步：

如果是回调或者Promise，比如最终要完成任务一，而任务一依赖任务二，任务二依赖任务三，则执行的过程为3-->2-->1，即首先获得3，2，才能完成最终的1。但是用async/await方式，则就为1->2->3了，实现了同步。**实现的方式就是使用await**，像同步一样执行任务，然后。。。。。:heavy_exclamation_mark:(好像有点问题哈)
