...
翻译：将输入的binary name（即Java类的全名）对应的类返回，前提是**当前加载器**已经被JVM记录为具有这个binary name的类的启动类，反之返回`null`。

注意上面加粗，只有被当前类加载器加载的才会返回哦，其他类加载器加载的是不会返回的。
...
用我的大白话讲，就是将一个接口的实现通过配置文件的形式装配，这一点至少思想上和IOC一直，只是实现的方式有所不同。
