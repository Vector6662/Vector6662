...
#### [泛型](https://segmentfault.com/a/1190000014120746)

- 把参数当作类型一样进行传递
- <数据类型>只能是引用类型

1. 主要难点还是[**泛型方法**](https://blog.csdn.net/weixin_43819113/article/details/91042598)
...
List<Stirng> as=new ArrayList<>();
test(as,ao);
```

[下面的引用至关重要！！！](https://blog.csdn.net/weixin_43819113/article/details/91042598)​ :smile: :exclamation: :exclamation::exclamation: ​

> 与类，接口中泛型参数不同的是，方法中的泛型参数无须**显式**传入实际类型参数，如上面程序所示，当程序调用 fromArrayToCollection() 方法时，无须在调用该方法前传入String、Object 等类型，但系统依然可以知道类型参数的数据类型，因为<u>编译器根据**实参**推断类型实参的值</u>，它通常推断出最直接的类型参数。

思考来自于Gson的用法之一[fromJson()](https://www.jianshu.com/p/bca8117ad49e)：
...
2. 还要注意**泛型类**：泛型类就是把泛型定义在类上，用户使用该类的时候，才把类型明确下来

   有一个发现：不论是泛型类和泛型方法，*首先都要在`<>`中对要使用的泛型通配符进行声明*，只是两者位置不一样而已

   同时泛型类声明的<T>是全局的，泛型方法内都可以使用，但是泛型方法声明的<T>只能在该方法内使用

3. [以及**泛型通配符**`?`和`T`的区别：](https://www.cnblogs.com/minikobe/p/11547220.html)
...
4. 对泛型的理解有些“肤浅”，不能仅仅将泛型理解为`List<T> list` 中的T，表示这个列表每个元素的类型而已，如果是这样的化`Class<T> clazz`就无法解释了。用`Class<T>`来理解泛型更好，因为T就仅仅是将类型参数化。
